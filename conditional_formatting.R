...
test_data <- data[,c("Base.Stat.Total", features_stats, "Base.Stat.Total", features_size, "Base.Stat.Total", "Base.Happiness", "Catch.Rate"),with=FALSE]
...
brks[,"Catch.Rate"] <- c(seq(0,45,length.out = num_brks/2),seq(45,255,length.out = num_brks/2-1))
...
for(i in c(0,1,2)){
  ...
  for(j in 1:nrow(test_data)){
    rowCallback <- c(
      ...
      if (j %in% c(1,8,11))
      {
        sprintf("$('td:eq(%d)',row).css('border-top', '1px solid Gainsboro').css('font-size', '10pt');", 
                i)
      }
